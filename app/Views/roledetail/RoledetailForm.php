...
<head>
    ...
    <title>Form Add/Edit Role Detail</title>
...
<body>
    ...
    <div class="main-container">
        ...
        $formAction = $isEdit ? base_url('roledetail/update/' . $roledetail['id_roledetail']) : base_url('roledetail/store');
        ?>

        <div class="container-fluid p-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h3>Form Role Detail</h3>
                <?php if ($isEdit): ?>
                    <a href="#" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#confirmDeleteModal"
                        onclick="setDeleteUrl('<?= base_url('roledetail/delete/' . $roledetail['id_roledetail']) ?>')">Delete Role Detail</a>
                <?php endif ?>
            </div>
            ...
            <div class="form-section">
                <form method="post" action="<?= $formAction ?>">
                    <div class="row">
                        <div class="col-md-6 form-group">
                            ...
                            <select name="id_menu" class="form-control" required>
                                <option value="">Select Role Detail Menu</option>
                                <?php foreach ($menus as $c): ?>
                                    <option value="<?= $c['id_menu'] ?>"
                                        <?= isset($isEdit) && $isEdit && $roledetail['id_menu'] == $c['id_menu'] ? 'selected' : '' ?>>
                                        <?= $c['id_menu'] ?> - <?= esc($c['nama_menu']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-6 form-group">
                            ...
                            <select name="id_role" class="form-control" required>
                                <option value="">Select Role Detail Role</option>
                                <?php foreach ($roles as $e): ?>
                                    <option value="<?= $e['id_role'] ?>"
                                        <?= isset($isEdit) && $isEdit && $roledetail['id_role'] == $e['id_role'] ? 'selected' : '' ?>>
                                        <?= $e['id_role'] ?> - <?= esc($e['judul_role']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div class="d-flex justify-content-center">
                        <?php if ($isEdit): ?>
                            <a href="<?= base_url('roledetail') ?>" class="btn-cancel">Cancel</a>
                        <?php endif ?>
